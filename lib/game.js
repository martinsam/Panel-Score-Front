...
Set.prototype.is_end = function(){
	// un joueur a  11 ou au dessus de 11 et deux points d'écart.
	if(this.point.get_score()[0] >= this.point_limit || this.point.get_score()[1] >=this.point_limit){
        if(Math.abs(this.point.get_score()[0] - this.point.get_score()[1]) >= this.point_gap){
            return true;
        }
        else{
            return false;
...
Match.prototype.is_end = function(){
	// le premier qui obtient trois sets gagnants
	if(this.point.get_score()[0] == this.set_to_win || this.point.get_score()[1] == this.set_to_win){
		return true;
	}
	return false;
};
